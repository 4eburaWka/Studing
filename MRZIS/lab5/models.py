...
class Autoencoder:
    def __init__(self, input_size, encoded_size):
        ...
        self.weights_encode = np.random.randn(encoded_size, input_size) * 0.01
        self.weights_decode = np.random.randn(input_size, encoded_size) * 0.01
        self.bias_encode = np.zeros((encoded_size, 1))
        self.bias_decode = np.zeros((input_size, 1))

    def encode(self, x):
        z = np.dot(self.weights_encode, x) + self.bias_encode
        return z

    def decode(self, z):
        x_hat = np.dot(self.weights_decode, z) + self.bias_decode
        return x_hat
    ...
    def backprop(self, x, lr=0.01):
        ...
        # Backward pass
        delta_x_hat = x_hat - x
        delta_z = np.dot(self.weights_decode.T, delta_x_hat)
        
        # Update weights and biases
        self.weights_encode -= lr * np.dot(delta_z, x.T)
        self.bias_encode -= lr * delta_z
        self.weights_decode -= lr * np.dot(delta_x_hat, z.T)
        self.bias_decode -= lr * delta_x_hat
